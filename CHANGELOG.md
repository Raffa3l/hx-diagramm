...
### Behoben
- **Responsive Layout für iPhone/iPad:** Die Seite war auf Mobilgeräten nicht
  vollständig bedienbar (fixe 300-px-Sidebar neben dem Diagramm,
  `overflow: hidden` verhinderte jedes Scrollen). Unterhalb 900 px Breite
  stapelt das Layout jetzt: Diagramm zuoberst in voller Breite
  (Höhe über `aspect-ratio` der viewBox), Bedienpanels scrollbar darunter.
- iOS-spezifisch: `100dvh` statt `100vh` (dynamische Adressleiste),
  Eingabefelder auf Mobilgeräten mit 16-px-Schrift (verhindert Auto-Zoom
  beim Fokussieren), `touch-action: none` auf Zustandspunkten
  (Touch-Drag verschiebt den Punkt statt die Seite zu scrollen).
...
- Sättigungslinie, Linien konstanter relativer Feuchte (10 bis 90 %),
